...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
    ...
    const completedVal =
      typeof task.completed === 'string'
        ? task.completed.toLowerCase() === 'yes' ||
          ...
          task.completed.toLowerCase() === 'completed'
        : !!task.completed;
    setIsCompleted(completedVal);
  }, [task.completed]);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE}/${task._id || task.id}`, {
        headers: getAuthHeaders(),
      });
      onRefresh?.();
    } catch (err) {
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      await axios.put(`${API_BASE}/${task._id || task.id}`, payload, {
        headers: getAuthHeaders(),
      });
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map((opt) => (
                  <button
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-xs sm:text-sm hover:bg-sky-100 flex items-center gap-2 transition-colors duration-200"
                  >
                    {opt.icon}
                    {opt.label}
                  </button>

...
const { tasks = [], refreshTasks = () => {} } = outletContext;
 ...
 const isTaskCompleted = (task) => {
  ...
  if (typeof task.completed === 'string') {
    const val = task.completed.toLowerCase().trim();
    return ['yes', 'true', 'completed'].includes(val);
  }
  return false;
};
      ...
      <div className={CT_CLASSES.header}>
        <div className={CT_CLASSES.titleWrapper}>
          <h1 className={CT_CLASSES.title}>
            ...
            <span className='truncate'>Complete Tasks</span>  
          </h1>
        ...
        {sortedCompletedTasks.length===0?(
          <div className={CT_CLASSES.emptyState}>
            <div className={CT_CLASSES.emptyIconWrapper}>
              <CheckCircle2 className='w-6 h-6 md:h-8 text-sky-500'/>
           </div> 
